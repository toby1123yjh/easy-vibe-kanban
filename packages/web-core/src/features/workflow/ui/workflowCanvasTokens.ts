...
export const WORKFLOW_CANVAS_NODE_SURFACE_CLASSES = {
  ...
  toolbar:
    'workflow-node-toolbar nodrag nopan absolute -top-9 right-2 z-20 flex items-center gap-1 rounded-lg border border-[var(--workflow-popover-border)] bg-[var(--workflow-popover-bg)] p-1 shadow-[var(--workflow-popover-shadow)] backdrop-blur transition-all duration-150',
  actionButton:
  ...
  addNext:
    'workflow-node-add-next nodrag nopan absolute -right-12 top-1/2 z-20 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full border border-brand/40 bg-[var(--workflow-popover-bg)] text-brand shadow-[var(--workflow-node-shadow-add-next)] backdrop-blur transition-all duration-150 hover:border-brand hover:bg-brand hover:text-white disabled:cursor-not-allowed disabled:opacity-40',
  handle:

...
<Note>
Approval and input controls are exposed according to the selected runtime's
capability snapshot. Unsupported capabilities fail explicitly instead of being
silently emulated.
</Note>
...
<Note>
Message editing and resume controls are exposed only when the selected
runtime advertises the corresponding native capability.
</Note>

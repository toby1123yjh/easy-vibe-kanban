...
- **unless-trusted** - Asks before actions Codex has not classified as trusted
...
<Tab title="CODEX">
  ...
  Approval level: `"unless-trusted"`, `"on-request"`, or `"never"`
  </ParamField>

  <ParamField path="model_reasoning_effort" type="string">
  Provider-native reasoning depth. Codex 0.144.1 models can advertise values such as `"low"`, `"medium"`, `"high"`, `"xhigh"`, `"max"`, or `"ultra"`.
  </ParamField>
...
<Tab title="OH_MY_PI">
  <ParamField path="model" type="string">
  Specify the model used by Oh My Pi
  </ParamField>

  <ParamField path="append_prompt" type="string | null">
  Add text to the provider-native prompt
  </ParamField>

  [View full documentation →](https://github.com/can1357/oh-my-pi)
</Tab>
</Tabs>
